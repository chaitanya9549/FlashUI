  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < files.length; i++) {
          ...
          const fileData = await new Promise<string>((resolve) => {
              reader.onload = (event) => {
                  ...
                  if (file.type.startsWith('image/')) {
                      // For images, we want the base64 part
          ...
          newFiles.push({
              ...
              type: file.type,
              data: fileData,
              previewUrl: file.type.startsWith('image/') ? `data:${file.type};base64,${fileData}` : undefined
          });
      }
...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleExport = () => {
      ...
      if (currentSession && focusedArtifactIndex !== null) {
          const artifact = currentSession.artifacts[focusedArtifactIndex];
          setDrawerState({ isOpen: true, mode: 'export', title: 'Export Code', data: artifact.html });
      }
  };
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
  ...
  }, [inputValue, isLoading, sessions.length, attachedFiles]);
            ...
            <div className="floating-input-container">
                    ...
                    <div className="attached-files-preview">
                        {attachedFiles.map(file => (
                            <div key={file.id} className="file-preview-item">
                                ...
                                <button className="remove-file" onClick={() => removeFile(file.id)}>&times;</button>
                            </div>
                        ))}
                    </div>
                )}
                <div className={`input-wrapper ${isLoading ? 'loading' : ''}`}>
                    <input 
                        ...
                        multiple 
                    />
                    <button className="attach-button" onClick={handleFileAttach} disabled={isLoading}>
                        <PaperclipIcon />
                    </button>
